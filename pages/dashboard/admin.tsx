...
import ShortenC from '../../components/AdminDashboard';
...
export default function Shorten() {
    ...
    useEffect(() => {
        if (router.asPath !== router.route) {
            if (!user || !user.ranks.admin) {
                router.push('/');
            }
        }
    }, [router, user]);

    if (!user || !user.ranks.admin) return null;

    if (!user || !user.ranks.admin) {
        router.push('/');
    }

    if (!user || !user.ranks.admin) return (<div className={css.container}></div>);
